...
const SidebarCard = (props: Props) => {
    ...
    const removeNoteHandler = (e: React.MouseEvent) => {
        ...
        dispatch(setActive({text: '', title: '', preview: ''}))
        e.stopPropagation()
    }

    return (
        <button className={cls.sidebarCard} onClick={clickCardHandler} aria-label="note in list">
            <h3>{note.title}</h3>
            <p>{note.preview}</p>
            <button className={cls.iconButton} onClick={(e) => {removeNoteHandler(e)}} aria-label="delete note">
